...
const sampleInput = `position=< 9,  1> velocity=< 0,  2>
position=< 7,  0> velocity=<-1,  0>
position=< 3, -2> velocity=<-1,  1>
position=< 6, 10> velocity=<-2, -1>
position=< 2, -4> velocity=< 2,  2>
...
const input = `position=<-41214, -10223> velocity=< 4,  1>
...
function calculateSkyDimensions(stars) {
...
function day10(input, maxHeight) {
    let stars = input.split('\n').map((line) => {
        const [x, y, dx, dy] = line.match(/position=< *(-?\d*), *(-?\d*)> velocity=< *(-?\d*), *(-?\d*)>/).slice(1, 5).map(n => parseInt(n));
        return {x, y, dx, dy};
    });

    let time = 0;
    let [minX, maxX, minY, maxY] = calculateSkyDimensions(stars);
    do {
        time += 1;
        stars.forEach(star => {
            star.x += star.dx;
        ...
        [minX, maxX, minY, maxY] = calculateSkyDimensions(stars);
    } while (maxY - minY > maxHeight);

    let sky = (new Array(maxY + 1 - minY)).fill(0)
        .map(r => new Array(maxX + 1 - minX).fill('.'));
    stars.forEach(star => {
        const {x, y} = star;
        sky[y - minY][x - minX] = '#';
    });
    console.log(sky.map(r => r.join('')).join('\n'));
    return time;
}

assert.equal(day10(sampleInput, 8), 3);
console.log(day10(input, 10));
