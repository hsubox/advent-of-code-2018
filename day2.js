...
function day2_part1(input) {
    let countMaps = input.split('\n')
        .map(s => {
            let countMap = {};
            for (let l in s) {
                l = s[l];
                if (!countMap.hasOwnProperty(l)) {
                    countMap[l] = 0;
                }
                countMap[l] += 1;
            }
    ...
    const exactly2Count = countMaps.filter((obj) => hasValue(obj, 2)).length;
    const exactly3Count = countMaps.filter((obj) => hasValue(obj, 3)).length;
    return exactly2Count * exactly3Count;
}
  
assert.equal(day2_part1('abcdef\nbababc\nabbcde\nabcccd\naabcdd\nabcdee\nababab'), 12);
console.log(day2_part1(input));
...
function day2_part2(input) {
    ...
    for (let i = 0; i < strs.length; i++) {
        for (let j = i + 1; j < strs.length; j++) {
            if (oneEditAway(strs[i], strs[j])) {
                return commonStr(strs[i], strs[j]);
            }
        }
    }
}
  
assert.equal(day2_part2(`abcde\nfghij\nklmno\npqrst\nfguij\naxcye\nwvxyz`), 'fgij');
console.log(day2_part2(input));
